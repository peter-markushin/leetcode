...
var reduceTestData = []reduceTest{
	...
	{[]int{8828, 9581, 49, 9818, 9974, 9869, 9991, 10000, 10000, 10000, 9999, 9993, 9904, 8819, 1231, 6309}, 134365, 16},
}

func TestMinOperations(t *testing.T) {
	for _, test := range reduceTestData {
		if result := MinOperations(test.Nums, test.X); result != test.Expected {
			t.Errorf("Expected %d for %+v, %d, but got %d", test.Expected, test.Nums, test.X, result)
		}
	}
}
...
func BenchmarkMinOperations(b *testing.B) {
	for i := 0; i < b.N; i++ {
		MinOperations(reduceTestData[3].Nums, reduceTestData[3].X)
	}
}
